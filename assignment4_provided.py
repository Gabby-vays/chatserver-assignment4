...
# Used throughout the program to format messages and safely send data to clients.def ok(msg: str) -> bytes:
    return (msg.rstrip() + "\n").encode()

def err(msg: str) -> bytes:
    return (f"ERR {msg}".rstrip() + "\n").encode()

def prompt(user: str, n: int) -> bytes:
    return f"<{user}:{n}> ".encode()
